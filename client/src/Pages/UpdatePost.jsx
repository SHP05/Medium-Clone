...
const UpdatePost = () => {
    ...
    const getPost = async () => {
        ...
        await axios.get(`http://localhost:3001/getpost/${pid}`)
            .then(result => {
                ...
                setPostImg(result.data.data.image);
            })
    ...
    const uploadPostImg = async (e) => {
        ...
        axios.put(`http://localhost:3001/uploadcoverimg/${pid}`, formdata, {
            ...
            <section className='bg-gray-950'>
                <div className='flex bg-gray-950 h-full'>
                    <div className="bg-gray-950 grow mx-20">
                        ...
                        <form onSubmit={uploadPostImg} className='bg-gray-950 text-lg flex flex-col w-2/3 mx-auto'>
                            <img src={`http://localhost:3001/postimg/${postimg}`} alt='post Image' className='w-52 h-52 rounded-2xl'></img>
                            <label className="block mb-1 font-medium text-gray-300" htmlFor="default_size">Update Cover Image for your Blog</label>
                            <input
                                name="image"
                                type="file"
                                accept='image/*'
                                ...
                                className="bg-gray-500/25 border h-16 border-gray-500 text-gray-200 text-lg rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500"
                                id="default_size"
                            />
                            <button type='submit' className='text-white'>Upload Cover Image</button>
                        </form>
